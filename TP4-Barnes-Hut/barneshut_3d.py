...
class Node:
    ...
    def into_next_quadrant(self):
        ...
        return self._subdivide(0) + 2 * self._subdivide(1) + 4 * self._subdivide(0)
...
def force_on(body, node, theta):
    ...
    # 2. Otherwise, calculate the ratio s/d. If s/d < theta, treat this internal
...
def verlet_bruteforce(bodies, G, dt):  # not used in the original code
    # to be completed
    pass


def plot_bodies_2D(bodies, i):
...
def init_momentum_2D(bodies,mass, inivel, ini_radius):
    for body in bodies:
        r = body.pos() - array([0.5, 0.5])
        body.momentum = array([-r[1], r[0]]) * mass * \
            inivel * norm(r) / ini_radius


def init_momentum_3D(bodies, mass, inivel, ini_radius):
    for body in bodies:
...
if __name__ =="__main__":
    ...
    init_momentum_3D(bodies, mass, inivel, ini_radius)
